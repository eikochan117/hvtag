...
use crate::{database::{db_loader::open_db, init, sql::{assign_tags_to_work, insert_error, insert_managed_folder, insert_tag}}, dlsite::assign_data_to_work, folders::{get_list_of_folders, get_list_of_unscanned_works}, tagger::types::WorkDetails};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in x {
        ...
        let res = assign_data_to_work(&db, i.clone()).await;
        match res {
            Ok(_) => println!("{i} ... done"),
